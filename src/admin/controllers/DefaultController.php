	...
	public function actionIndex()
	{
		...
		if(Yii::$app->request->isPost){
			...
			if ($isValid) {
				...
				if($mode == $currentMode) {
					\App::$domain->navigation->alert->create(['offline/main', 'mode_' . $modeStr . '_already_selected'], Alert::TYPE_WARNING);
				} else {
					\App::$domain->navigation->alert->create(['offline/main', 'success_' . $modeStr], Alert::TYPE_SUCCESS);
				}
				return $this->refresh();
			}
		} else {
